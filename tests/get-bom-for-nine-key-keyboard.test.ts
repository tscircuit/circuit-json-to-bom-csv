...
test("get-bom-for-nine-key-keyboard", async () => {
  ...
  const validCircuitJson = nineKeyKeyboardCircuitJson.filter((component: any) => 
  ...
  const key = bomRows.find((row) => 
    row.supplier_part_number_columns?.["JLCPCB Part #"] === "C5184526"  // Ensure the correct part number
  );
  // Check if key is found
  expect(key).toBeDefined();
  expect(key?.designator).toBeDefined();
  ...
  const diode = bomRows.find((row) => 
    row.supplier_part_number_columns?.["JLCPCB Part #"] === "C57759"
  );

  expect(diode).toBeDefined();
  expect(diode?.designator).toBeDefined();
  ...
  const keyRows = bomRows.filter((row) => row.supplier_part_number_columns?.["JLCPCB Part #"] === "C5184526");
  ...
  const diodeRows = bomRows.filter((row) => row.supplier_part_number_columns?.["JLCPCB Part #"] === "C57759");
